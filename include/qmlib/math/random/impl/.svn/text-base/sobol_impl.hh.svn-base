...
#		include <qmlib/math/random/primpoly.hpp>
	...
	inline qm_real lowDiscrepacySequenceGenerator::point(qm_Size n, qm_Size dim) const  {
...
inline HaltonGenerator::HaltonGenerator(qm_Size dim, qm_Size seed):lowDiscrepacySequenceGenerator(dim,1)  {
}
	
inline void HaltonGenerator::nextSequence() const  {
	qm_uns_long n = this->sequenceCounter() + 1;
	...
	template<int D>
    inline SobolGenerator<D>::SobolGenerator(qm_Size dim, qm_Size seed):lowDiscrepacySequenceGenerator(dim,0),m_lastsequence(dim)  {
		...
		for(qm_Size i=0;i<m_dimensionality;i++) m_directionIntegers[i].resize(c_bits);
		//
		// initializes coefficient array of the k-th primitive polynomial
		// and degree of the k-th primitive polynomial
		std::vector<qm_uns_int>     degree(m_dimensionality);
		std::vector<qm_long>        ppmt(m_dimensionality);
		// degree 0 is not used
		ppmt[0]   = 0;
		degree[0] = 0;
		qm_Size k, index;
		qm_uns_int currentDegree;
		...
		//
		qm_Size maxTabulated = m_dimensionality;
	...
	template<int D>
    inline void SobolGenerator<D>::nextSequence() const  {
        qm_uns_long n = this->sequenceCounter() + 1;
        QM_REQUIRE(n > 0, "Period exceeded: overflow");
        ...
        int j = 0;
        while (n & 1) { n >>= 1; j++; }
        for (qm_Size k=0; k<m_dimensionality; k++) {
